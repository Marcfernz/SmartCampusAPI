...
### Part 5.5 Filters vs Manual Logging
 used a LoggingFilter class that implements both ContainerRequestFilter and ContainerResponseFilter. This automatically logs the HTTP method, URL and response status code for every request without touching any of the resource classes. If I had manually added Logger.info() inside every method across RoomResource, SensorResource and SensorReadingResource it would have been repetitive and easy to forget in some places. The filter is registered once and runs on everything automatically which keeps the resource classes clean and focused on their actual job.
